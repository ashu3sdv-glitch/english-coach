...
const SCENARIOS = [
  { id: "daily", label: "Everyday Talk", icon: "☕", unlockXp: 0, prompt: "Let's have a casual conversation about daily life. Ask me about my morning routine, hobbies, or what I did today. Keep it natural and friendly. I'm learning English, so please correct my mistakes gently after I respond." },
  { id: "travel", label: "Travel", icon: "✈️", unlockXp: 0, prompt: "Simulate a travel situation — I'm at an airport or hotel. Play the role of staff. Keep it realistic. Gently correct my English mistakes." },
  { id: "business", label: "Business", icon: "💼", unlockXp: 60, prompt: "Let's practice business English. Simulate a professional meeting scenario. I'm pitching a project idea. Ask me questions a manager would ask. Correct my grammar if needed." },
  { id: "music", label: "AI Music", icon: "🎵", unlockXp: 60, prompt: "Let's discuss AI music production and Suno AI. You're a fellow creator. Ask me about my workflow, what music I make, and share your thoughts. This topic is close to my heart. Correct my English naturally." },
  { id: "interview", label: "Job Interview", icon: "🤝", unlockXp: 150, prompt: "You're a job interviewer. Ask me 3-5 interview questions one at a time. After each answer, give brief feedback on my English and then move to the next question." },
  { id: "story", label: "Storytelling", icon: "📖", unlockXp: 250, prompt: "Let's do a collaborative story. Start a short story and I'll continue it. We alternate. Gently correct my grammar and vocabulary. Keep the story fun and imaginative." },
];
...
// XP is stored in localStorage so it persists across sessions and other modules
// (e.g. streak) can eventually read it too.
function useChatXp() {
...
// Parses an assistant reply into: main English text, Russian hint, and correction note.
// The model is instructed (see system prompt) to always append a line starting with
// "RU:" (Russian translation of its own message) and, optionally, a line starting
// with "CORRECTION:" (feedback on the learner's previous message).
...
function ChatModule() {
  ...
  const XpBar = () => {
    ...
    return (
      <div style={{ position: "relative", background: "#0d1117", border: "1px solid #ffffff15", borderRadius: 12, padding: "10px 14px", marginBottom: 16 }}>
        ...
        {xpGain != null && (
          <div style={{ position: "absolute", right: 14, top: -10, color: "#00ff88", fontSize: 13, fontWeight: 700, animation: "none" }}>
            +{xpGain} XP
          </div>
        )}
      </div>
  ...
  if (!scenario) return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {SCENARIOS.map((sc) => {
          const locked = xp < sc.unlockXp;
          return (
            <button key={sc.id} onClick={() => !locked && startScenario(sc)} disabled={locked}
              style={{ background: "#0d1117", border: "1px solid #ffffff15", borderRadius: 14, padding: "16px 14px", cursor: locked ? "not-allowed" : "pointer", textAlign: "left", transition: "all 0.2s", opacity: locked ? 0.5 : 1, position: "relative" }}
              onMouseOver={(e) => { if (!locked) { e.currentTarget.style.border = "1px solid #00ff8840"; e.currentTarget.style.background = "#0a1a0f"; } }}
              onMouseOut={(e) => { e.currentTarget.style.border = "1px solid #ffffff15"; e.currentTarget.style.background = "#0d1117"; }}>
              <div style={{ fontSize: 24, marginBottom: 6 }}>{locked ? "🔒" : sc.icon}</div>
              <div style={{ color: "#fff", fontSize: 13, fontWeight: 600 }}>{sc.label}</div>
              {locked && <div style={{ color: "#888", fontSize: 11, marginTop: 4 }}>откроется на {sc.unlockXp} XP</div>}
            </button>
          );
        })}
      </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: 520 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, paddingBottom: 12, borderBottom: "1px solid #ffffff10" }}>
        ...
        {messages.map((m, i) => {
          if (m.role === "user") {
            return (
              <div key={i} style={{ display: "flex", justifyContent: "flex-end" }}>
                <div style={{ maxWidth: "82%", padding: "12px 16px", borderRadius: "18px 18px 4px 18px", background: "linear-gradient(135deg, #1a3a2a, #0d2a1a)", border: "1px solid #00ff8830", color: "#e0e0e0", fontSize: 14, lineHeight: 1.6, whiteSpace: "pre-wrap" }}>
                  {m.content}
                </div>
              </div>
            );
          ...
          return (
            <div key={i} style={{ display: "flex", justifyContent: "flex-start" }}>
              <div style={{ maxWidth: "82%", padding: "12px 16px", borderRadius: "18px 18px 18px 4px", background: "#161b22", border: "1px solid #ffffff10", color: "#e0e0e0", fontSize: 14, lineHeight: 1.6, whiteSpace: "pre-wrap" }}>
                <div>{parsed.en}</div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", fontFamily: "system-ui, -apple-system, sans-serif", color: "#fff", paddingBottom: 40 }}>
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "14px 20px 0" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr", gap: 6, marginBottom: 22 }}>
          {tabs.map((t) => (
